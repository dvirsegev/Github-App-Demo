...
MICROSERVICE_PATH =  "apps/" #os.environ.get("MICROSERVICE_PATH")
# ../../yarn.lock
...
def get_deploy_config(ms_name, token):
    ...
    deployment_config, status = get_port_entity(
        "DeploymentService", identifier, token)
    if status != 200 and status != 201:
        print(f"DeploymentService named {identifier} doesn't exist!")
        return None
...
def create_packages_for_dc(ms_name, token, yarn_dict):
    ...
    Returns:
                    dict: Will return a dict consisting of a "DeploymentService" port entity, with the updated packages.
                                            If the requested "DeploymentService" entity doesn't exist, return None.
    """
    print(
        f"########## Creating Package entities for {ms_name} DeploymentService! ##########")
...
def main():
    ...
    for ms_name in os.listdir(MICROSERVICE_PATH):
        ...
        if dc_entity is not None:
            report_to_port("DeploymentService", dc_entity, token)
